...
const MARKUP_TOOLS = [
  { kind: 'highlight', label: '荧光笔', menuLabel: '荧光高亮', Icon: Highlighter },
  { kind: 'underline', label: '下划线', menuLabel: '下划线', Icon: Underline },
  { kind: 'strikeout', label: '删除线', menuLabel: '删除线', Icon: Strikethrough },
] as const;
...
const Thumbnail = memo(function Thumbnail({ pdf, page, selected, onClick }: { pdf: PDFDocumentProxy; page: number; selected: boolean; onClick: () => void }) {
  ...
  return <button ref={buttonRef} className={`pdf-thumbnail ${selected ? 'is-current' : ''}`} onClick={onClick} title={`前往第 ${page} 页`} aria-label={`第 ${page} 页缩略图`}>
...
function OutlineTree({ items, selected, onSelect, collapsed, onToggle, level = 0 }: {
  ...
  return <ul className="pdf-outline-tree" role={level ? 'group' : 'tree'}>{items.map(item => <li key={item.id} role="treeitem" aria-selected={selected === item.id} aria-expanded={item.children.length ? !collapsed.has(item.id) : undefined}>
    <div className={`pdf-outline-row ${selected === item.id ? 'is-selected' : ''}`} style={{ paddingLeft: `${8 + level * 13}px` }}>
      {item.children.length ? <button className="pdf-icon-button pdf-outline-toggle" title={collapsed.has(item.id) ? '展开章节' : '折叠章节'} onClick={() => onToggle(item.id)}>{collapsed.has(item.id) ? <ChevronRight size={12} /> : <ChevronDown size={12} />}</button> : <span className="pdf-outline-spacer" />}
      <button className="pdf-outline-link" title={`${item.title} · 第 ${item.page} 页`} onClick={() => onSelect(item)}><span>{item.title}</span><small>{item.page}</small></button>
    </div>
...
export default function PdfPane(props: PdfPaneProps) {
  ...
  const annotationLayers = useRef(new WeakMap<HTMLElement, { signature: string; rotation: number; layer: HTMLElement }>());
  ...
  const [status, setStatus] = useState('正在打开 PDF…');
  ...
  const copyText = async (text: string) => {
    ...
    } catch (error) { props.onError(`无法复制文字：${messageOf(error)}`); }
  };
  ...
  const renderAnnotations = useCallback((onlyPage?: number) => {
    ...
    for (const pageView of views) {
      ...
      if (cached?.signature === group.signature && cached.rotation === pageView.viewport.rotation && cached.layer.parentElement === pageView.div) continue;
      cached?.layer.remove();
      ...
      for (const annotation of group.items) {
        annotation.rects.forEach((rect, index) => {
          ...
          if (index === 0) {
            ...
            button.title = `${annotation.comment || '查看标记笔记'}（右键可编辑或删除批注）`;
            button.setAttribute('aria-label', annotation.comment || '查看标记笔记');
      ...
      annotationLayers.current.set(pageView.div, { signature: group.signature, rotation: pageView.viewport.rotation, layer });
    }
  }, [openAnnotation]);

  useEffect(() => { setOutline(paper.outline); }, [paper.outline]);
  useEffect(() => { renderAnnotations(); }, [paper.annotations, renderAnnotations]);
  ...
  useEffect(() => {
    ...
    setPdf(null); setFailure(''); setStatus('正在打开 PDF…'); setPageCount(0); setSelection(null); setLocalHighlightArmed(false); setNewComment(false); setPasswordRequest(null); setOutlineSelection(null); setAnnotationId(null); setSearch(''); setMatches({ current: 0, total: 0 }); setIndexing(false); setContextMenu(null); setCopied(false);
    ...
    eventBus.on('pagerendered', ({ pageNumber, error }: { pageNumber: number; error?: Error }) => {
      if (!disposed) { renderAnnotations(pageNumber); if (error) latest.current.onError(`第 ${pageNumber} 页渲染失败：${error.message}`); }
    });
    ...
    void (async () => {
      ...
      loadingTask.onPassword = (update: (password: string) => void, reason: number) => {
        if (disposed) return;
        setStatus('需要 PDF 密码'); setPassword('');
        setPasswordRequest({ update, incorrect: reason === PasswordResponses.INCORRECT_PASSWORD, cancel: () => { setPasswordRequest(null); setStatus(''); setFailure('已取消打开加密 PDF。'); void loadingTask?.destroy(); } });
      };
      ...
      if (disposed) return;
      setIndexing(false);
      if (viewer.pdfDocument) { latest.current.onError(`PDF 已打开，但文章文字索引未完成：${messageOf(error)}`); return; }
      setStatus(''); setPasswordRequest(null); setFailure(messageOf(error));
    });
    return () => {
  ...
  }, [workspaceId, paper.id, attempt, renderAnnotations]);
  ...
  const addOutline = () => {
    ...
    const item: OutlineItem = { id: crypto.randomUUID(), title: `第 ${page} 页`, page, children: [] };
  ...
  const editOutline = () => {
    ...
    if (found) { found.siblings[found.index].title = outlineTitle.trim() || '未命名章节'; found.siblings[found.index].page = clamp(Number(outlinePage) || 1, 1, pageCount); saveOutline(next); }
  };
  ...
  const annotationTools = (annotationToolbar !== 'selection' || selection) && <div className={`pdf-selection-tools pdf-annotation-tools-${annotationToolbar}${sharedToolbar ? ' pdf-annotation-tools-shared' : ''}`} role="toolbar" aria-label="文字与批注工具" aria-busy={annotationBusy} onMouseDown={event => event.preventDefault()}>
    {newComment && selection ? <form onMouseDown={event => event.stopPropagation()} onSubmit={event => { event.preventDefault(); saveMarkup(markupKind, commentDraft); }}>
      <label>给这段文字添加批注<textarea autoFocus aria-label="新批注" value={commentDraft} onChange={event => setCommentDraft(event.target.value)} placeholder="你的想法、疑问或发现…" /></label>
      <div><button type="button" className="pdf-text-button" onClick={() => { setNewComment(false); setSelection(null); setCommentDraft(''); }}>取消</button><button className="pdf-text-button" disabled={annotationBusy}><Check size={14} />保存</button></div>
    </form> : <>
      <button title="选择和复制文字，退出文字标记模式" aria-label="选择文字" aria-pressed={!highlightArmed} className={!highlightArmed ? 'is-on' : ''} disabled={!pdf} onClick={() => setHighlightArmed(false)}><MousePointer2 size={15} /></button>
      <div className="pdf-highlight-colors" role="group" aria-label="标记颜色">{COLORS.map(color => <button key={color} style={{ backgroundColor: color }} className={highlightColor === color ? 'is-chosen' : ''} onClick={() => setHighlightColor(color)} title={`高亮颜色 ${color}`} aria-label={`高亮颜色 ${color}`} aria-pressed={highlightColor === color} />)}</div>
      {MARKUP_TOOLS.map(({ kind, label, Icon }) => {
        const chosen = highlightArmed && markupKind === kind;
        return <button key={kind} title={chosen ? `${label}已开启：选中文字自动标记，再点一次或按 Esc 退出` : `开启${label}，然后选中文字；已有选中文字时立即标记`} aria-label={label} aria-pressed={chosen} className={chosen ? 'is-on' : ''} disabled={!pdf || annotationBusy} onClick={() => chooseMarkup(kind)}><Icon size={15} />{label}</button>;
      })}
      <button title={`复制选中文字（${shortcutLabel('C')}）`} aria-label="复制选中文字" disabled={!selection} onClick={() => void copyText(selectedText() || selection?.text || '')}>{copied ? <Check size={15} /> : <Copy size={15} />}{copied ? '已复制' : '复制'}</button>
      <button title={selection ? '给选中文字添加批注' : '先选中文字，再添加批注'} disabled={!selection || annotationBusy} onClick={() => startComment(selection)}><MessageSquare size={15} />批注</button>
      <button title={selection ? '针对选中文字向 AI 提问' : '先选中文字，再向 AI 提问'} className="pdf-ask-button" disabled={!selection} onClick={() => { if (selection) props.onAsk({ ...selection.parts[0], text: selection.text }); }}><Sparkles size={15} />问 AI</button>
      <button title={annotationToolbar === 'fixed' ? '移到页面底部悬浮' : '固定到顶部栏'} aria-label="切换工具栏位置" disabled={!props.onAnnotationToolbarChange} onClick={() => props.onAnnotationToolbarChange?.(annotationToolbar === 'fixed' ? 'floating' : 'fixed')}><MoveVertical size={15} /></button>
      {annotationBusy && <span className="pdf-markup-saving" role="status">正在保存标记…</span>}
      {annotationToolbar === 'selection' && <button title="关闭选中文字工具" aria-label="关闭选中文字工具" onClick={() => { setSelection(null); setHighlightArmed(false); }}><X size={14} /></button>}
    </>}
  </div>;

  return <section ref={paneRef} className={`pdf-pane ${active ? 'is-active' : ''} reading-${readingTheme}`} onPointerDown={props.onActivate} aria-label={`${paper.name} 阅读器`}>
    <div className="pdf-document-heading"><FileText size={14} /><span title={paper.name}>{paper.name}</span><small>{paper.role === 'main' ? '正文' : '补充材料'}</small></div>
    <div className="pdf-toolbar" role="toolbar" aria-label="PDF 阅读工具">
      <button className={`pdf-icon-button ${sidebar ? 'is-on' : ''}`} title="目录、缩略图与批注" aria-label="目录、缩略图与批注" aria-pressed={!!sidebar} onClick={() => setSidebar(sidebar ? null : 'outline')}><PanelLeft size={17} /></button>
      <button className={`pdf-icon-button ${searchOpen ? 'is-on' : ''}`} title={`搜索文章文字（${shortcutLabel('F')}）`} aria-label="搜索文章文字" onClick={() => setSearchOpen(!searchOpen)}><Search size={17} /></button>
      <span className="pdf-toolbar-divider" />
      <div className="pdf-page-control">
        <button className="pdf-icon-button" title="上一页" aria-label="上一页" disabled={!pdf || page <= 1} onClick={() => viewerRef.current?.previousPage()}><ChevronLeft size={16} /></button>
        <form onSubmit={event => { event.preventDefault(); goToPage(Number(pageInput)); setPageInput(String(clamp(Number(pageInput) || page, 1, pageCount || 1))); }}><input aria-label="当前页码" title="输入页码并回车跳转" value={pageInput} inputMode="numeric" onChange={event => setPageInput(event.target.value.replace(/[^0-9]/g, ''))} onBlur={() => setPageInput(String(page))} /><span>/ {pageCount || '—'}</span></form>
        <button className="pdf-icon-button" title="下一页" aria-label="下一页" disabled={!pdf || page >= pageCount} onClick={() => viewerRef.current?.nextPage()}><ChevronRight size={16} /></button>
      </div>
      <span className="pdf-toolbar-divider" />
      <div className="pdf-zoom-control" role="group" aria-label="此阅读区的独立缩放">
      <button className="pdf-icon-button pdf-zoom-step" title="缩小此阅读区" aria-label="缩小" disabled={!pdf} onClick={() => viewerRef.current?.decreaseScale()}><Minus size={15} /></button>
      <select className="pdf-zoom-select" aria-label="缩放比例" value={['page-width', 'page-fit', 'auto', '0.5', '0.75', '1', '1.25', '1.5', '2', '3'].includes(scale) ? scale : 'custom'} disabled={!pdf} onChange={event => { if (viewerRef.current && event.target.value !== 'custom') viewerRef.current.currentScaleValue = event.target.value; }}>
        <option value="page-width">适合宽度</option><option value="page-fit">适合页面</option><option value="auto">自动缩放</option>
      ...
      <button className="pdf-icon-button pdf-zoom-step" title="放大此阅读区" aria-label="放大" disabled={!pdf} onClick={() => viewerRef.current?.increaseScale()}><Plus size={15} /></button>
      </div>
      <div className="pdf-toolbar-spacer" />
      <button className={`pdf-icon-button ${optionsOpen ? 'is-on' : ''}`} title="阅读模式与导出" aria-label="阅读模式与导出" aria-expanded={optionsOpen} onClick={() => setOptionsOpen(!optionsOpen)}><Settings2 size={17} /></button>
    {optionsOpen && <div className="pdf-options">
      <div className="pdf-options-title"><strong>阅读设置</strong><button className="pdf-icon-button" aria-label="关闭阅读设置" onClick={() => setOptionsOpen(false)}><X size={14} /></button></div>
      <label>滚动方式<select aria-label="滚动方式" value={scrollMode} disabled={!pdf} onChange={event => { if (viewerRef.current) viewerRef.current.scrollMode = Number(event.target.value); }}><option value={0}>连续垂直</option><option value={1}>连续水平</option><option value={2}>多行排列</option><option value={3}>单页翻阅</option></select></label>
      <label>页面排列<select aria-label="页面排列" value={spreadMode} disabled={!pdf || scrollMode === 1} onChange={event => { if (viewerRef.current) viewerRef.current.spreadMode = Number(event.target.value); }}><option value={0}>单栏</option><option value={1}>双页</option><option value={2}>双页 · 独立封面</option></select></label>
      <label>自定缩放<form onSubmit={event => { event.preventDefault(); const input = event.currentTarget.elements.namedItem('zoom') as HTMLInputElement; const zoom = Number(input.value); if (viewerRef.current && Number.isFinite(zoom)) viewerRef.current.currentScaleValue = String(clamp(zoom, 10, 1000) / 100); }}><input key={scalePercent} name="zoom" type="number" min="10" max="1000" defaultValue={scalePercent} aria-label="自定缩放百分比" /><span>%</span><button className="pdf-icon-button" title="应用缩放" aria-label="应用缩放" disabled={!pdf}><Check size={14} /></button></form></label>
      <button className="pdf-menu-action" disabled={!pdf} onClick={() => { if (viewerRef.current) viewerRef.current.pagesRotation = (viewerRef.current.pagesRotation + 90) % 360; }}><RotateCw size={15} />顺时针旋转 90°</button>
      <div className="pdf-options-separator" />
      <button className="pdf-menu-action" disabled={!!busy} onClick={() => void exportOrPrint('export')}>{busy === 'export' ? <LoaderCircle className="pdf-spin" size={15} /> : <Download size={15} />}导出 PDF（含目录与批注）</button>
      <button className="pdf-menu-action" disabled={!!busy} onClick={() => void exportOrPrint('print')}>{busy === 'print' ? <LoaderCircle className="pdf-spin" size={15} /> : <Printer size={15} />}打印 PDF</button>
    </div>}
    </div>

    {searchOpen && <div className="pdf-search-bar">
      <div className="pdf-search-input"><Search size={14} /><input ref={searchRef} aria-label="搜索 PDF" placeholder="在文章中查找…" value={search} onChange={event => setSearch(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); find('again', event.shiftKey); } }} /><span aria-live="polite">{findPending ? '查找中' : search ? `${matches.current} / ${matches.total}` : ''}</span></div>
      <button className={`pdf-icon-button pdf-search-case ${caseSensitive ? 'is-on' : ''}`} title="区分大小写" aria-label="区分大小写" aria-pressed={caseSensitive} onClick={() => setCaseSensitive(!caseSensitive)}>Aa</button>
      <button className={`pdf-icon-button pdf-search-word ${wholeWord ? 'is-on' : ''}`} title="完整单词" aria-label="完整单词" aria-pressed={wholeWord} onClick={() => setWholeWord(!wholeWord)}>ab</button>
      <button className="pdf-icon-button" title="上一个结果" aria-label="上一个结果" onClick={() => find('again', true)}><ArrowUp size={14} /></button><button className="pdf-icon-button" title="下一个结果" aria-label="下一个结果" onClick={() => find('again')}><ArrowDown size={14} /></button>
      <button className="pdf-icon-button" title="关闭搜索" aria-label="关闭搜索" onClick={() => setSearchOpen(false)}><X size={14} /></button>
    </div>}

    {!sharedToolbar && annotationToolbar === 'fixed' && annotationTools}
    <div className="pdf-pane-body">
      {sidebar && <aside className="pdf-sidebar" aria-label="PDF 导航">
        <div className="pdf-sidebar-tabs">
          <button className={sidebar === 'outline' ? 'is-on' : ''} onClick={() => setSidebar('outline')} title="目录" aria-label="目录"><ListTree size={16} /></button>
          <button className={sidebar === 'thumbnails' ? 'is-on' : ''} onClick={() => setSidebar('thumbnails')} title="缩略图" aria-label="缩略图"><FileText size={16} /></button>
          <button className={sidebar === 'annotations' ? 'is-on' : ''} onClick={() => setSidebar('annotations')} title="高亮与批注" aria-label="高亮与批注"><StickyNote size={16} />{!!paper.annotations.length && <small>{paper.annotations.length}</small>}</button>
        </div>
        <div className="pdf-sidebar-caption"><span>{sidebar === 'outline' ? '文章目录' : sidebar === 'thumbnails' ? '页面缩略图' : '高亮与批注'}</span>{sidebar === 'outline' && <button className="pdf-icon-button" title="将当前页加入目录" aria-label="将当前页加入目录" disabled={!pdf || props.editBusy} onClick={addOutline}><BookmarkPlus size={15} /></button>}</div>
        <div className="pdf-sidebar-scroll">
          {sidebar === 'outline' && (outline.length ? <OutlineTree items={outline} selected={outlineSelection} onSelect={chooseOutline} collapsed={collapsed} onToggle={id => setCollapsed(previous => { const next = new Set(previous); next.has(id) ? next.delete(id) : next.add(id); return next; })} /> : <div className="pdf-sidebar-empty"><ListTree size={25} /><p>还没有文章目录</p><span>在当前页添加章节，建立自己的阅读索引。</span><button className="pdf-text-button" onClick={addOutline} disabled={!pdf || props.editBusy}><Plus size={13} />添加当前页</button></div>)}
          ...
          {sidebar === 'annotations' && (!paper.annotations.length ? <div className="pdf-sidebar-empty"><Highlighter size={25} /><p>留住阅读中的灵感</p><span>选中 PDF 文字，即可高亮、批注或向 AI 提问。</span></div> : [...paper.annotations].sort((a, b) => a.page - b.page || a.createdAt - b.createdAt).map(annotation => <button key={annotation.id} className={`pdf-annotation-card ${annotationId === annotation.id ? 'is-selected' : ''}`} onClick={() => { openAnnotation(annotation); goToPage(annotation.page); }} style={{ '--annotation-color': annotation.color } as CSSProperties}><small>第 {annotation.page} 页 {annotation.comment && '· 批注'}</small><blockquote>{annotation.text || '页面批注'}</blockquote>{annotation.comment && <p>{annotation.comment}</p>}</button>))}
        </div>
        {sidebar === 'outline' && selectedOutline && <fieldset className="pdf-outline-editor" disabled={props.editBusy} aria-label="编辑目录项"><label>章节名称<input aria-label="章节名称" value={outlineTitle} onChange={event => setOutlineTitle(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') editOutline(); }} /></label><label className="pdf-outline-page-label">指向页码<input type="number" aria-label="目录目标页码" min={1} max={pageCount} value={outlinePage} onChange={event => setOutlinePage(event.target.value)} /><button className="pdf-icon-button" title="保存目录项" aria-label="保存目录项" onClick={editOutline}><Check size={15} /></button></label>
          <div className="pdf-outline-actions">{([['up', '上移章节', ArrowUp], ['down', '下移章节', ArrowDown], ['outdent', '提升层级', ArrowLeft], ['indent', '缩进层级', ArrowRight], ['delete', '删除章节及子目录', Trash2]] as const).map(([action, title, Icon]) => <button key={action} className="pdf-icon-button" title={title} aria-label={title} disabled={(action === 'up' || action === 'indent') ? selectedOutline.index === 0 : action === 'down' ? selectedOutline.index === selectedOutline.siblings.length - 1 : action === 'outdent' ? !selectedOutline.parent : false} onClick={() => { saveOutline(changeOutline(outline, outlineSelection!, action)); if (action === 'delete') setOutlineSelection(null); }}><Icon size={14} /></button>)}</div>
        </fieldset>}
        {sidebar === 'annotations' && selectedAnnotation && <div className="pdf-comment-editor"><div className="pdf-markup-editor-options"><label>标记类型<select aria-label="批注类型" value={annotationKindDraft} onChange={event => setAnnotationKindDraft(event.target.value as MarkupKind)}>{MARKUP_TOOLS.map(item => <option key={item.kind} value={item.kind}>{item.menuLabel}</option>)}</select></label><label>颜色<input type="color" aria-label="批注颜色" value={annotationColorDraft} onChange={event => setAnnotationColorDraft(event.target.value)} /></label></div><label>批注<textarea ref={annotationInputRef} aria-label="批注内容" placeholder="记录你的想法…" value={commentDraft} onChange={event => setCommentDraft(event.target.value)} /></label><div className="pdf-comment-actions"><button className="pdf-text-button pdf-delete-annotation" title="删除这条批注及标记，可撤销" aria-label="删除批注" disabled={annotationBusy} onClick={() => deleteAnnotation(selectedAnnotation.id)}><Trash2 size={14} />删除批注</button><button className="pdf-text-button" disabled={annotationBusy} onClick={() => { if (submitAnnotations(latest.current.document.annotations.map(item => item.id === annotationId ? { ...item, kind: annotationKindDraft, color: annotationColorDraft, comment: commentDraft } : item))) setAnnotationId(null); }}><Check size={13} />保存批注</button></div></div>}
      </aside>}
      <div className="pdf-canvas-area">
        <div ref={containerRef} className={`pdf-viewer-container ${highlightArmed ? 'pdf-highlighter-active' : ''}`} tabIndex={0} aria-label="PDF 页面" onMouseUp={event => { if (event.button === 0 && !event.ctrlKey) captureSelection(true); }} onKeyUp={event => { if (event.shiftKey || event.key === 'Shift') captureSelection(event.key === 'Shift'); }} onContextMenu={event => { event.preventDefault(); openContextMenu(event.clientX, event.clientY, event.target); }} onKeyDown={event => {
          ...
          if (/^https?:|^mailto:/i.test(href)) void window.folio.openExternal(href).catch(error => props.onError(messageOf(error)));
        }}><div ref={viewerElementRef} className="pdfViewer" /></div>
        {!!status && !passwordRequest && <div className="pdf-loading"><LoaderCircle size={25} className="pdf-spin" /><span>{status}</span></div>}
        {!!failure && <div className="pdf-loading pdf-error"><FileText size={30} /><strong>暂时无法打开这份 PDF</strong><p>{failure}</p><button className="pdf-text-button" onClick={() => setAttempt(attempt + 1)}>重新打开</button></div>}
        {!sharedToolbar && annotationToolbar !== 'fixed' && annotationTools}
        {copied && <div className="pdf-copy-feedback" role="status"><Check size={13} />已复制到剪贴板</div>}
        {passwordRequest && <div className="pdf-password-backdrop"><form className="pdf-password-dialog" role="dialog" aria-modal="true" aria-labelledby={`password-title-${paper.id}`} onSubmit={event => { event.preventDefault(); passwordRequest.update(password); setPasswordRequest(null); setStatus('正在解锁 PDF…'); setPassword(''); }}><LockKeyhole size={27} /><h3 id={`password-title-${paper.id}`}>这份 PDF 已加密</h3><p>{passwordRequest.incorrect ? '密码不正确，请重新输入。' : '输入文档密码，继续阅读。密码不会保存。'}</p><input ref={passwordInputRef} type="password" aria-label="PDF 文档密码" value={password} onChange={event => setPassword(event.target.value)} autoComplete="off" /><div><button type="button" className="pdf-text-button" onClick={passwordRequest.cancel}>取消</button><button className="pdf-text-button pdf-primary-button" disabled={!password}>打开 PDF</button></div></form></div>}
      </div>
    </div>
    <footer className="pdf-status-bar"><span>{indexing ? `正在准备文章索引 ${indexProgress}%` : paper.textStatus === 'empty' ? '扫描文档 · 暂无可提取文字' : `${pageCount || '—'} 页 · 本地阅读`}</span><span>{readingTheme === 'sepia' ? '护眼纸色' : readingTheme === 'dark' ? '夜间阅读' : '原色显示'}<i />{scalePercent}%</span></footer>
    {sharedToolbar && active && props.annotationToolbarHost && createPortal(annotationTools, props.annotationToolbarHost)}
    {contextMenu && createPortal(<div ref={contextMenuRef} className="folio-pdf-context-menu" role="menu" aria-label="PDF 操作" style={{ left: contextMenu.x, top: contextMenu.y }} onMouseDown={event => event.preventDefault()} onContextMenu={event => event.preventDefault()} onKeyDown={event => {
      ...
      {contextAnnotation && <>
        <button type="button" role="menuitem" disabled={annotationBusy} onClick={() => { openAnnotation(contextAnnotation); closeContextMenu(); requestAnimationFrame(() => annotationInputRef.current?.focus({ preventScroll: true })); }}><MessageSquare size={15} /><span>编辑批注</span></button>
        <button type="button" role="menuitem" className="pdf-context-danger" disabled={annotationBusy} onClick={() => { deleteAnnotation(contextAnnotation.id); closeContextMenu(true); }}><Trash2 size={15} /><span>删除批注</span></button>
        <div role="separator" />
      </>}
      <button type="button" role="menuitem" aria-label="复制选中文字" disabled={!contextMenu.text.trim()} onClick={() => { void copyText(contextMenu.text); closeContextMenu(true); }}><Copy size={15} /><span>复制选中文字</span><kbd>{shortcutLabel('C')}</kbd></button>
      <button type="button" role="menuitem" disabled={!contextMenu.selection || annotationBusy} onClick={() => { startComment(contextMenu.selection); closeContextMenu(); }}><MessageSquare size={15} /><span>添加批注</span></button>
      <div role="separator" />
      ...
      <div role="separator" />
      <button type="button" role="menuitem" disabled={!props.onSplit} onClick={() => { closeContextMenu(); props.onSplit?.('vertical'); }}><Columns2 size={15} /><span>左右分栏</span></button>
      <button type="button" role="menuitem" disabled={!props.onSplit} onClick={() => { closeContextMenu(); props.onSplit?.('horizontal'); }}><Rows2 size={15} /><span>上下分栏</span></button>
      <button type="button" role="menuitem" disabled={!props.onSplit} onClick={() => { closeContextMenu(); props.onSplit?.('none'); }}><FileText size={15} /><span>取消分栏</span></button>
